...
from gaussian_renderer import render_ori, render_Balance,render_Smooth
import torchvision
...
def render_set(model_path, load2gpt_on_the_fly, name, iteration, views, gaussians, pipeline, background, deform):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        alpha = results["alpha"]
        rendering_1 = torch.clamp(torch.cat([results["render"], alpha]), 0.0, 1.0)
        ...
        alex_lpips_list.append(lpips(image[None], gt_image[None], net_type='alex'))  #
        lpips_list.append(lpips(image[None], gt_image[None], net_type='vgg'))
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool, mode: str, load2device_on_the_fly=False):
    with torch.no_grad():
        ...
        gaussians = GaussianModel_ST(dataset.sh_degree, fea_dim=gs_fea_dim, with_motion_mask=dataset.gs_with_motion_mask)
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False,loader=args.dataset_type)
