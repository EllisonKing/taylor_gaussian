...
from gaussian_renderer import network_gui, render_flow,render_ori,render_Balance,render_Smooth
import sys
from scene import Scene, GaussianModel, DeformModel,GaussianModelSmooth,GaussianModelBalance
...
class GUI:
    def __init__(self, args, dataset, opt, pipe, testing_iterations, saving_iterations) -> None:
        ...
        self.deform = DeformModel(K=self.dataset.K, deform_type=self.dataset.deform_type, is_blender=self.dataset.is_blender, skinning=self.args.skinning, hyper_dim=self.dataset.hyper_dim, node_num=self.dataset.node_num, 
                      ...
                      enable_densify_prune=self.opt.node_enable_densify_prune, is_scene_static=dataset.is_scene_static,model_path=model_path,flag_=flag)
    ...
    def train_step(self):
        ...
        if not self.viewpoint_stack:
            ...
            # self.trainCameras = self.scene.getTrainCameras().copy() # add
            # self.testCameras = self.scene.getTestCameras().copy() # add
    ...
    def train_node_rendering_step(self):
        # Pick a random Camera
        # print('self.viewpoint_stack0:', self.viewpoint_stack)
        if not self.viewpoint_stack:
            if self.opt.progressive_train_node and self.iteration_node_rendering < int(self.opt.progressive_stage_steps / self.opt.progressive_stage_ratio) + self.opt.node_warm_up:
                if self.iteration_node_rendering < self.opt.node_warm_up:
                    sorted_train_cams = sorted(self.scene.getTrainCameras().copy(), key=lambda x: x.fid)
                    print('sorted_train_cams:',sorted_train_cams)
        ...
        with torch.no_grad():
            if self.iteration_node_rendering < self.opt.iterations_node_sampling:
                ...
                if strategy == 'direct':
                    ...
                    print('trans_samp :',trans_samp.size())
                    print('original_gaussians.get_xyz[:, None] :',original_gaussians.get_xyz[:, None].size())
...
def add_args_to_config(config=None, lp=None, op=None, pp=None,cfg=None):
    config.ModelParams.update(vars(lp))
...
if __name__ == "__main__":
    ...
    # config.additional_params=cfg
    config = add_args_to_config(config=config, lp=lp, op=op, pp=pp,cfg=cfg)
