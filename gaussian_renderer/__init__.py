...
# from diff_gaussian_rasterization import  GaussianRasterizationSettings as GaussianRasterizationSettings_
# from diff_gaussian_rasterization import GGaussianRasterizer as GaussianRasterizer_
...
def render_ori(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, d_xyz, d_rotation, d_scaling,
               d_opacity=None, d_color=None, scaling_modifier=1.0, override_color=None, random_bg_color=False,
               render_motion=False, detach_xyz=False, detach_scale=False, detach_rot=False, detach_opacity=False,
               d_rot_as_res=True, scale_const=None, d_rotation_bias=None, force_visible=False):
    """
    Render the scene.
...
def render_Smooth(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, d_xyz, d_rotation, d_scaling,
             d_opacity=None, d_color=None, scaling_modifier=1.0, override_color=None, random_bg_color=False,
             render_motion=False, detach_xyz=False, detach_scale=False, detach_rot=False, detach_opacity=False,
             d_rot_as_res=True, scale_const=None, d_rotation_bias=None, force_visible=False):
    """
    ...
    gaussian_times = torch.ones((pc.get_xyz.shape[0], 1), dtype=pc.get_xyz.dtype, requires_grad=False, device="cuda") + 0  #
    key_time = pc.get_key_time
    get_time_scale = pc.get_time_scale
    _opacity_ = pc.get_opacity if d_opacity is None else pc.get_opacity + d_opacity  # add ！！！

    exp_f = exp_func
    delta_time = viewpoint_camera.timestamp * gaussian_times - key_time  # xt-u
    f_delta_time = delta_time / torch.exp(get_time_scale)
    exp_time = exp_f(f_delta_time)

    opacity = _opacity_ * exp_time
    pc.exp_time = exp_time

    delta_t = delta_time.detach()

    f_1d_motion = pc._f_n_motion[:, 0:3]  #∂f/∂t
    f_2d_motion = pc._f_n_motion[:, 3:6]  # ∂²f/∂t²
    f_3d_motion = pc._f_n_motion[:, 6:9]  # # ∂³f/∂t³

    means3D = means3D + (f_1d_motion * delta_t) + (f_2d_motion * delta_t **2) + (f_3d_motion * delta_t**3)
    pc_get_features = pc.get_features
    ...
    if render_motion:
        ...
        colors_precomp = None
        if colors_precomp is None:
            sh_features = torch.cat([pc_get_features[:, :1] + d_color[:, None], pc_get_features[:, 1:]],
                                    dim=1) if d_color is not None and type(d_color) is not float else pc_get_features
            if pipe.convert_SHs_python:
                shs_view = sh_features.transpose(1, 2).view(-1, 3, (pc.max_sh_degree + 1) ** 2)
                dir_pp = (pc.get_xyz - viewpoint_camera.camera_center.repeat(sh_features.shape[0], 1))
                dir_pp_normalized = dir_pp / dir_pp.norm(dim=1, keepdim=True)
    ...
    ## add rotations and scale taylor series
    f_1d_scale = pc._scale_param[:, 0:3] #∂f/∂t
    f_2d_scale = pc._scale_param[:, 3:6] # ∂²f/∂t²
    f_3d_scale = pc._scale_param[:, 6:9] # # ∂³f/∂t³
    scales = scales + (f_1d_scale * delta_t) + (f_2d_scale * delta_t**2) + (f_3d_scale * delta_t**3)

    # f_1_roattion = pc._f_n_rotation[:, 0:3]
    rotations = pc.get_rotation(delta_t)
...
def render_Balance(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, d_xyz, d_rotation, d_scaling,
               d_opacity=None, d_color=None, scaling_modifier=1.0, override_color=None, random_bg_color=False,
               render_motion=False, detach_xyz=False, detach_scale=False, detach_rot=False, detach_opacity=False,
               d_rot_as_res=True, scale_const=None, d_rotation_bias=None, force_visible=False):
    """
    ...
    if pipe.compute_cov3D_python:
        ...
        scales = pc.get_scaling + d_scaling
        rotations = pc._rotation + d_rotation
        # rotations = pc.get_rotation_bias(d_rotation)
        # if d_rotation_bias is not None:
        #     rotations = quaternion_multiply(d_rotation_bias, rotations)
    ...
    ## add rotations and scale taylor series, Compute derivatives of various orders
    f_1d_scale = pc._scale_param[:, 0:3]
    f_2d_scale = pc._scale_param[:, 3:6]
    f_3d_scale = pc._scale_param[:, 6:9]
    scales = scales + (f_1d_scale * delta_t) + (f_2d_scale * delta_t ** 2) + (f_3d_scale * delta_t ** 3)

    f_1d_rotation = pc._f_n_rotation
    rotations = rotations + f_1d_rotation * delta_t
