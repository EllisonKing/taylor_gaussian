...
def interpolate_point(pcd, N=4):
    ...
    #
    # return newpcd
    return newxyz,newcolor,newnormal,newtime
...
class keman_select(nn.Module):
    def __init__(self):
        super(keman_select, self).__init__()
        pass
    ...
    def forward(self,x, n_classes):
        ...
        for class_label in trange(n_classes):
            # 获取属于该类别的点的索引
            class_indices = np.where(labels == class_label)[0]
            class_points = x[class_indices]

            # 计算该类别的中心
            center = kmeans.cluster_centers_[class_label]

            # 计算每个点到中心的距离
            distances = np.linalg.norm(class_points - center, axis=1)

            # 找到距离中心最近的点的索引
            closest_point_index = class_indices[np.argmin(distances)]
...
class ControlNodeWarp(nn.Module):
    def __init__(self, is_blender, init_pcl=None, node_num=512, K=3, use_hash=False, hash_time=False, enable_densify_prune=False, pred_opacity=False, pred_color=False, with_arap_loss=False, with_node_weight=True, local_frame=False, d_rot_as_res=True, skinning=False, hyper_dim=2, progressive_brand_time=False, max_d_scale=-1, is_scene_static=False, flag_=0,model_path=None,**kwargs):
        ...
        self.hyper_dim = hyper_dim if not skinning else 0  # skinning should not be with hyper self.hyper_dim = 8
        self.is_blender = is_blender
        self.pred_opacity = pred_opacity
        self.pred_color = pred_color
        self.max_d_scale = max_d_scale
    ...
    def init(self, opt, init_pcl, hyper_pcl=None, keep_all=False, force_init=False, as_gs_force_with_motion_mask=False, force_gs_keep_all=False, reset_bbox=True, **kwargs):
        ...
        if keep_all or self.node_num > init_pcl.shape[0]:
            ...
            if self.flag ==0:
                ...
                init_nodes_idx = farthest_point_sample(pcl_to_samp.detach()[None], self.node_num)[0]
                self.nodes.data = nn.Parameter(torch.cat(
                    [init_pcl[init_nodes_idx].float(),
                     1e-2 * torch.ones([self.node_num, self.hyper_dim]).float().cuda()],
                    dim=-1))
     
                with open(self.model_path +'_data.pkl', 'wb') as f:
                    pickle.dump(self.nodes.data, f)
                ...
                init_nodes_idx = torch.tensor(loaded_array).to('cuda:0')
    ...
    def elastic_loss(self, t=None, delta_t=0.005, K=2, t_samp_num=8):
        ...
        node_trans = self.node_deform(t=t_samp)['d_xyz']
        nodes_t = self.nodes[:, None, :3].detach() + node_trans  #M, T, 3  nodes_t:(512,8,3)  self.nodes[:, None, :3]:(512,1,3) node_trans:(512,8,3)

        # Calculate weights of nodes NN
        #  nn_weight:[512,3] nn_idx: [512,3]
        nn_weight, _, nn_idx = self.cal_nn_weight(x=self.nodes[..., :3].detach(), feature=self.nodes[..., 3:], K=K+1)
        nn_weight, nn_idx = nn_weight[:, 1:], nn_idx[:, 1:]  # M, K nn_weight:[512,2] nn_idx: [512,2]
